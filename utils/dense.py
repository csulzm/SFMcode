...
def denseMatch(graph,imA,imB,imSA,imSB):
    ...
    znccs = []
    for i in range(2):
        #Here should be the image in the range of 0-1 (CV2 leaves 0-256)
...
def propagate(i_m,mim_i,mim_j,zncc_i,zncc_j,winhalfsize):
    ...
    for match_pair_size in range(match_pair.shape[0]):
        e1 = zncc_i[int(match_pair[int(match_pair_size),0]),int(match_pair[int(match_pair_size),1]),:]
        e2 = zncc_j[int(match_pair[int(match_pair_size),2]),int(match_pair[int(match_pair_size),3]),:]
        val = np.sum(e1*e2)
...
def reliableArea(im):
    permARow = list(range(1,im.shape[0])) + list([0])
    permACol = list(range(1,im.shape[1])) + list([0])

    permBRow = list([im.shape[0] - 1]) + list(range(0,im.shape[0]-1))
    permBCol = list([im.shape[1] - 1]) + list(range(0, im.shape[1] - 1))
...
def outputPly2(savefile,points,text=True):
    
    points = [(points[i,0], points[i,1], points[i,2]) for i in range(points.shape[0])]
    vertex = np.array(points, dtype=[('x', 'f4'), ('y', 'f4'),('z', 'f4')])
    el = PlyElement.describe(vertex, 'vertex', comments=['vertices'])
    PlyData([el], text=text).write(savefile)
