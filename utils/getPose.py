...
def getPose(E,K,matches,ims):
    ...
    if np.linalg.det(R2) < 0:
        print ('Negative determinant R2 times negative 1.')
        R2 = -1 * R2

    #This leads to 4 possible solutions
    t1t = t1.reshape(3,1)
    t2t = t2.reshape(3,1)
...
def vgg_X_from_xP_nonlin(u,P,imsize=None,X=None):
    ...
    #If it's not proportional, I get x first.
    if (X is None):
        X = vgg_X_from_xP_lin(u, P, imsize)
    #Fixed el-1??? (in the old version, the index is different from the number)
    newu = u.copy()-1
    newP = P.copy()
...
def formatVgg(X): #Must be a 3-element vector
    row = [0,X[2],-X[1]]
    row2 = [-X[2],0,X[0]]
    row3 = [X[1],-X[0],0]
